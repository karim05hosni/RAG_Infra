...
from repositories import text_search_ts_rank, qdrant_search
from shared import transformer


def keywordSearch(query, use_bm25=False, use_ts_postgres=True):
    keyword_search_results = []
    ts_rank = text_search_ts_rank(query)
...
def RFF(bm25_scores, dot_product_scores, k=60):
...
def hybrid_search(query: str, use_qdrant_sem_search=True, use_bm25=False, use_ts_postgres=True):
    ...
    keyword_search_scores = keywordSearch(query)
    RFF_scores = RFF(keyword_search_scores, semantic_search_scores)

    # get chunks texts from postgres
    chunk_ids = [chunk_id for chunk_id, score in RFF_scores]
    chunks = fetch_chunks_by_ids(chunk_ids)
    final_results = []
    for chunk_id, score in RFF_scores:
